...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        data=await getTask()
        if(data.code===100){
            console.log('今日抽奖结果:京豆'+data.beanNum+'个,奖券编号:'+data.lotteryNo+' 已完成')
        }else{
            data1 = await GameSign()
            if(data1.status===true){
                data=await doTask()
                if(data.code===100){
                    console.log('今日抽奖结果:京豆'+data.beanNum+'个,奖券编号:'+data.lotteryNo)
                }
            }else{
